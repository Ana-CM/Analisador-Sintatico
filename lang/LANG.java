...
public class LANG extends Grammar {
    public LANG(String input, Actions actions) {
        this.input = input;
    ...
    public static TreeNode parse(String input, Actions actions) throws ParseError {
        LANG parser = new LANG(input, actions);
        return parser.parse();
    }
    ...
    private TreeNode parse() throws ParseError {
        TreeNode tree = _read_Prog();
        if (tree != FAILURE && offset == inputSize) {
            return tree;
        }
        if (expected.isEmpty()) {
            failure = offset;
            expected.add(new String[] { "LANG", "<EOF>" });
        }
